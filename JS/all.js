...
// banner
  ...
  var carAreaWidth = parseInt($(".carArea").css("width"));
  var vectorWidth = parseInt($(".carArea .img_vector").css("width"));
  var carWidth = parseInt($(".carArea .img_car").css("width"));
  ...
  ScrollTrigger.create({
    ...
    //向下滾動進入start點時觸發callback
    onEnter: function () {
      gsap.to(".img_car",{x :() =>{return (carWidth + vectorWidth) - carAreaWidth},duration:3});
    },

    //向下滾動超過end點時觸發callback
    onLeave: function () {
      gsap.set(".img_car" ,  {x :() =>{return carAreaWidth - (carWidth + vectorWidth)}});
    }, 
